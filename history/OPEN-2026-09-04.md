# Open Questions, as they stood on 2026-09-04

Rotated out of \OPEN.md\ when it reached 150 items and 956 lines. Read-only
evidence: quote it to answer a named question, cite the section, never browse
it for context. 78 of these 150 blocked on \episodes\, \derive\, \interviewor nothing at all.
...
- The generator renders a form for a class the map says is computed.
  ...
  Answered 4 Sep: the filter reads the absence of a `designates_type` slot — 19
  of the 21 classes carry one, and the two that do not are exactly these. Left
  as a line only until a form has actually been refused. Found 4 Sep rendering
  all 21 forms · blocks: -
...
- The generator's row rule is a guess and the log now contradicts it. Every one
  ...
  `designates_type: true` on `entity_class`. Answered 4 Sep: the rule reads that
  flag and walks `is_a`, measured at `Ingredient` 81 and `Location` 42 where a
  strict reading gives 4 and 9. Left as a line only until the change has
  actually run. Found 4 Sep entering 197 master-data rows · blocks: -
...
- A form submitted without its class field mints an entity that is a row of
  nothing, and the 1 Sep line said `submit()` would prevent exactly this. That
  line reasoned that `submit()` already receives `class_name` and spends it on
  `action_name` alone, so stating the class is one added assertion per minted
  entity and stays domain-blind — it learns that a form came from class X, so
  what it mints is an X. What exists instead is an `entity_class` slot the map
  declares and the form asks for like any other field, carrying no
  `required: true`; the master-data pass set it explicitly on all 197. Since
  4 Sep that field decides whether an entity is visible in any table at all, so
  the gap stopped being cosmetic and became the difference between an entity
  and an invisible one. Whether `submit()` writes the class from the form's own
  class, whether the map marks the slot required, or both, is undecided — the
  first is code and the second is a new map version. Found 4 Sep reading the
  row-rule result · blocks: generation
...
- What serves the HTML, given that `generate.py` is a CLI with no service in it.
  The 4 Sep decision makes milestone one's forms a browser, so something has to
  answer a POST and call `submit()`. Three candidates and they differ only in
  what they cost the repo: `http.server` from the standard library adds no
  dependency and hand-rolls form parsing; Flask adds one dependency and removes
  that work; FastAPI adds several and buys nothing here, since there is no API
  to document and no async to exploit. This is thirty minutes of trying rather
  than an argument — write the smallest thing that renders one list, one form
  and accepts one POST, and the choice makes itself. What is not a T1 is the
  dependency itself, which is Fareza's under the CLAUDE.md rule that a new
  service or dependency means stop and ask. Raised 4 Sep · blocks: generation

- `pytest tests/generator` run directly writes into the working log. The suite
  seals its fixture map and submits through it, and it reads `UNITI_DSN`, whose
  default is `uniti`; only `make check` points at `uniti_check`. One run on
  4 Sep put 45 fixture assertions and 11 submissions into the log holding
  Sorella — every URI in the fixture's own `uniti:g_` namespace, nothing about
  the 197 entities changed, and the log is append-only so they stand. It was
  reported rather than hidden, which is the behaviour wanted, but a done
  condition saying no assertion would be written was breached by a test run and
  not by the change under test. Whether a conftest refuses a DSN it was not
  given, whether the fixtures get a database of their own, or whether
  `make check` is simply declared the only sanctioned way to run the suite, is
  thirty minutes of trying. Found 4 Sep checking the item's done condition ·
  blocks: -
